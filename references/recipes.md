# photo-doodle 主题配方与构图经验

五个经过验证的主题模板（源自真实项目迭代），以及构图/避坑经验。坐标全部是归一化 (0..1)，需按目标照片调整位置。

> ⚠️ **这五个配方是构图骨架，不是可直接选用的菜单。**
> 其中天使/猫咪/魔法三个都偏少女向，元素池高度重叠（sparkle、heart、star 反复出现），**照搬会直接导致所有照片长得一样**——真实事故：三张风格完全不同的女生照片被三次独立调用做成了同一套"撒花撒星 + golden hour 草写"。
> 用法：先按 SKILL.md 第 2 步从内容清单定主导元素家族，再来这里借鉴对应的构图手法（怎么让皇冠贴合头发、翅膀怎么分层、气泡文字怎么排）。**先定家族，后查手法，不要反过来**。
> 花草类元素（`flower`/`dotflower`）尤其容易滥用——除非人物与植物直接互动（手里拿花、坐在花丛里），否则不要撒在脸侧/肩上当通用点缀。

## 主题配方

### 1. angel 天使
- `halo(头顶x, 头顶上方y)` + 光环上叠 `stroke(brush='glitter', color=YELLOW)` 金粉
- 光环下方垫一条 `brush='water', color=LEMON` 短笔画做光晕
- 左右肩外侧画翅膀：一条上缘长弧线（w≈5.5）+ 3-4 段扇贝形下缘（under=False 避免叠影）+ 2 条羽轴 taper='tip'
- 翅膀内垫淡紫/粉水彩衬色（w≈22-26）
- `sparkle` 每颗都要落在照片里真实的光点/高光上（**别"四散"撒——那是套路本身，问题在均匀撒而不在数量**）+ 指尖/手边 `heart`（要她真的在指什么才画）
- **一版只许一句文字，英文 + 手写体**（`fname='marker'/'hand'/'note'`，配 `stroke_w=2.5` 加粗）+ `underline`（slope 匹配文字 rot，宽度用 `W_FAT`）；文案按 SKILL.md 硬禁止清单选，**`angel`/`golden hour` 这类抽象词不合格**

### 2. kitty 猫咪（"把人变成小猫"，对标小红书/抖音那种贴纸滤镜）

这一版在真实项目里完整验证过（`~/Documents/photos/doodles/v3-cat/`），**每个元素都锚在实测坐标上**，所以是少女向骨架里最不容易收敛的一个：

- `cat_ears_sticker(lx, ly, rx, ry, w_=, h_=)` 戴在**逐行实测的发际线**上（不是头顶上方）。用实心三角款，别用 `stroke` 描折线——圆头笔尖会磨圆尖角，做出来是兔耳。尺寸按头宽定：耳根宽 ≈ 头宽 60-65%
- 猫须**从照片里真实的东西长出去**才有意思：她按在脸上的叶子、举着的吸管、耳边的发夹。找不到锚点就贴着实测肤色边界起笔（别骑在手指上）
- 猫鼻只在**正脸**成立；四分之三侧脸贴上去会糊住鼻子，直接删
- 腮红：三道短斜线 `brush='crayon', color=PINK`，比圆圈更像动漫腮红
- **毛绒尾巴 `fur(pts, w=68)` 是全图的视觉重心**（那一笔特粗）。落位前必须逐行扫出主体的右缘（见 SKILL.md"大元素落位前"那节），眼估必错。衣服/裙摆是可用画布
- `paw_print` 顺**实测的真实地面纹理**（台阶亮边、砖缝、木地板接缝）一路退向画外，逐个变小 + `rot` 逐个微转，读作"猫刚从那边走过来"
- `blob` 铺一团白雾 + `emo_face` 手画颜文字压在上面（**别用字体写颜文字**，理由见 SKILL.md）
- 空地放 `fish`（朝着尾巴的方向游，猫在追鱼）+ `pixel_sprite` 像素小猫贴纸
- 一句英文手写文案，说她正在做的事：`leaf = whiskers` 这种

### 3. magic 魔法
- `crown` 戴头顶（水彩打底 → 描金 → glitter 撒金粉，三层工艺过程视频里很好看）
- 指尖 `sparkle(color=YELLOW)` 大星 + 两颗小星
- 魔法轨迹：长曲线 `color=HOTPINK, color2=SKY` 渐变 + 同路径 glitter 叠一遍，沿途 `star5`
- 天空 `arc` 两道同心月牙（YELLOW）+ 柠檬水彩光晕
- 大面积留白处用 `dashed(color=SKY)` 连星座，节点放 sparkle
- 草写文案 + 淡紫 underline（**"make a wish" 这类通用愿望词不合格**——换成这张照片里真在发生的事，比如她许愿的场合、当天日期）

### 4. rain 雨天治愈（适合伞/雨景照片）
- 伞外三色雨丝（BLUE/SKY/LAVENDER 轮换，短斜线 taper='tip'）
- 伞面上 `heart` ×3（不同颜色/角度）+ 每颗下方一道小弹跳弧 arc(200,340)
- 角落 `rainbow` 和 `sun`
- 伞下 `music_note`（淡紫/天蓝）
- 台阶/地面画发芽小苗（MINT 茎 + 两片叶）+ 天蓝水彩小水洼
- 标题 + 天蓝 underline

### 5. journal 手账标注（信息量最少但最百搭）
- `frame()` 手绘边框 + 两角 `tape`（半透明粉/天蓝斜贴，盖住边框角）
- `arrow` 指向照片里的真实细节 + 彩色标注短语（这是本主题的灵魂：找照片里值得吐槽/标注的物件）
- `arc(brush='highlight', color=LEMON)` 荧光圈重点物 + 描线
- 角落日期 + 心情词（fname='hand'）+ 蜡笔 underline + 一排四色 `dot` 颜料点

## 补充骨架（非少女向，平衡上面五个的偏向）

这四个来自真实交付，用于避免"所有照片都被做成少女风"。同样是骨架不是菜单。

### 6. 旅行手账（户外风景、登顶、打卡）
- 大字标题 `text(fname='hand')` + 副标短句 + `underline`（highlight 或 crayon）
- `dashed` 画路线，从画面下缘的起点（加个 `dot` 或小旗标记）蜿蜒连到照片里真实的地标/山脊——**必须落在真实地形上**，凭空一条线会像误画
- `arrow` + 短标注指向照片里可指认的细节（墨镜里的反射、山谷公路、远处的建筑）
- `arc(brush='highlight')` 荧光圈重点物 + 描线
- 角落画小指南针（双 `arc` + `poly` 指针）、小旗、`sun`、三笔 `^^^` 飞鸟
- 主导家族：标注手账 / 天气自然

### 7. 太空舱（呼应衣服图案、夜景、科幻感）
- 行星（`arc` 椭圆 + 倾斜环 + 表面小 `dot`）、火箭（`poly` 机身 + 三角尾翼 + `stroke` 尾焰）、UFO、`dashed` 轨道弧
- `star4` 点缀，每颗要有落点理由，别均匀撒
- `arrow` + 标注呼应衣服上的印花、随身物件
- 主导家族：器物呼应 / 星辰光

### 8. 科技注解（演讲、工作、发布会等纪实场景）
- 灯泡（`arc` 灯泡轮廓 + `stroke` 灯丝 + 四周短射线）、齿轮（`arc` + 一圈短 `stroke` 齿）、WiFi 三道同心 `arc`
- `speech_bubble` 放在说话人嘴边，内写主题关键词
- `arrow` + 标注指向真实物件（麦克风、笔记本、海报上的日期）
- `underline` 荧光标出关键信息 + 一排四色 `dot` 收尾
- 分寸感是关键：绝不用皇冠/爱心/撒花；**不许遮挡照片自带的文字信息**（海报、幻灯、字幕）
- 主导家族：标注手账 / 器物呼应

### 9. 光影几何（逆光、都市、极简、硬朗）
- 顺着照片真实光线方向画光束（`stroke` 长直线 taper='tip'，2-4 条平行）
- 几何框（`poly` 三角/菱形/方框，只描边）框住主体或留白区
- `dot` 阵列、`dashed` 网格线
- 单一强调色 + 白，克制到极致
- 主导家族：几何图形 / 星辰光

## 构图经验（踩坑总结）

- **别挡脸**：五官区域只放极轻的元素（腮红、胡须）；粗笔画、文字、水彩一律避开面部中心
- **贴着人画**：光环/皇冠/猫耳要"戴"在头发轮廓上（间隙 ≤0.02），翅膀贴肩膀外缘——离太远像贴纸不像涂鸦
- **文字放低对比区**：优先深色头发、阴影、裙摆上；亮背景上靠 under 底影保证可读
- **underline 的 slope 要匹配文字 rot**：rot=-10 的文字配 slope≈+0.02（x0→x1 向下），否则下划线和文字不平行
- **水彩衬色宽度 w≈20-26 就够**：太宽会糊成一片色斑
- **元素密度**：一版 10-18 个元素组；留白多的照片（天空/墙面）多放，主体占满的照片少放但不低于 8
- **笔画粗细必须分层**（`audit()` 硬闸）：用 `W_HAIR`/`W_THIN`/`W_MED`/`W_BOLD`/`W_FAT` 梯子，至少 3 档，且至少一笔 ≥ `W_BOLD`。真实反馈"线条太细、全是细的"——元素够了却仍显虚，根源就是这个。特粗物件（毛绒尾巴、大色块）直接传数字，`w=70` 那种
- **sparkle 的 w 参数**：留 None 自动算；手填过大会把整图刷白（真实翻车案例）
- **顺序即动画**：ops 记录顺序就是视频里的绘制顺序，按"主体装饰 → 环境点缀 → 文字收尾"排列最像真人涂鸦
- **贴纸感 ≠ 细线感**：复刻贴纸风设计时用饱满形状（`star4` 实心四角星、`crown_sketch` 圆圈尖顶皇冠、`cat_ears_sticker` 实心三角猫耳），细线 `sparkle`/`crown` 气质完全不同；复刻前裁局部放大比对形状神韵
- **实心形状要给 `poly(..., wob=1.2~1.5)`**：不给会得到激光切割般的完美几何形，一眼看出是程序画的
- **浅色画在亮背景上必须 `under=True`**：MINT/PEACH/LEMON 靠深色底影才立得住
- **复刻类任务**（照抄某张设计稿的元素布局）走 `references/design-replica.md` 标准流程：叠网格量归一化坐标 → 矢量复刻 → 并排裁块比对迭代

## 每版标准产出

```python
d = Doodle(SRC, seed=<每版不同>)
# ... 涂鸦 ...
d.save(f'{OUT}/v1-angel.jpg')       # 静态成品
d.save_video(f'{OUT}/v1-angel.mp4') # 逐笔过程视频（自动排时间轴）
```

视频规格：h264/yuv420p/30fps/faststart，时长自动 = 笔画总时长 + intro 0.6s + hold 1.6s（通常 10-15s）。渲染约 40-90s/条（视图片分辨率，带手约 +50%），多版本时可用 `run_in_background` 并行。

## 手部叠加层（hand overlay）

`save_video()` 默认 `hand=True, hand_sprite='anime', hand_scale=0.20`：二次元少女手握素描铅笔，笔尖精确锚定当前绘制点。机制要点：
- sprite 四选一：'anime'（默认，白皙女生手+铅笔，用户明确偏好）/ 'pencil'（欧美卡通手+铅笔）/ 'marker'（粉马克笔）/ 'drawn'（程序拳头，笔杆颜色逐笔跟随 op 颜色，每色缓存一次）；或自备 (png, tipx, tipy)
- hand_scale：手伸展半径 = min(W,H)×该值；0.20 低调，0.30 醒目（用户反馈 0.30 偏大）
- 笔画间隙抬笔：沿正弦弧线飞到下一笔起点，sprite 微放大 7% 模拟离纸
- text op 的笔尖沿文字擦入右边缘走（斜排文字方向也正确）
- 手臂朝画面右下伸出；排版时尽量避免把收尾元素放在画面最右下角（手飞出方向），否则出场动画会盖住它
