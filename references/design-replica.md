...
⚠️ **先看这条，否则这一步纯属浪费**：旧版本的提示词里逐个点名了"皇冠、爱心、星星、花朵、手写英文短语"，结果生图模型**照单点菜**返回 14 颗星 + 6 朵花 + "Sunshine girl"——那不是模型滑向套路，是提示词把套路点了出来。**提示词里绝对不要列具体图形清单**，改成描述任务约束、让它自己从照片内容里找梗：
...
codex exec --skip-git-repo-check --sandbox workspace-write -C /tmp/ai-doodle \
  ...
  "在这张照片上加手绘涂鸦装饰，输出涂鸦后的完整图片。要求：
   1. 每个涂鸦元素都必须呼应照片里真实存在的东西或正在发生的事——先说出你在照片里看到了哪 3-5 件能指名道姓的具体东西（物件、动作、光线落点），再只画呼应它们的元素；
   2. 禁止套路：不要撒星星、不要撒小花、不要在脸周围围一圈装饰、不要写 golden hour / sweet / lovely / sunshine 这类换任何照片都能用的短语；
   3. 文字若有，必须写出关于这张照片的一件具体事（地点、事件、日期、动作、心里话），不能只写物件名；
...
让它同时回答"每个元素呼应哪一件"有两个用处：一是逼它先看照片再画（而不是套模板），二是它的回答直接就是你第 3 步复刻时的对照清单。**若它的回答里出现"星星、花朵、可爱装饰"这类泛泛词而说不出呼应关系，说明这张稿子没价值，弃用、改走自行设计。**
