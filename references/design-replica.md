...
⚠️ **提示词里不要列具体图形清单**。旧版本点名了"皇冠、爱心、星星、花朵、手写英文短语"，生图模型就**照单点菜**返回 14 颗星 + 6 朵花 + "Sunshine girl"——那不是模型滑向套路，是提示词把套路点了出来。

反过来逐个禁具体图形（"不要星星、不要花"）也不要写：那仍然是在拿具体内容当指令（只是反向的），而且会误伤——她手里真捧着花的照片，画花本来是对的。**正确做法只提一条正向要求：每个元素都必须有照片依据。** 没依据的通用装饰自然被排除，有依据的花和星则被允许，判据统一。
...
codex exec --skip-git-repo-check --sandbox workspace-write -C /tmp/ai-doodle \
  ...
  "在这张照片上加手绘涂鸦装饰，输出涂鸦后的完整图片。要求：
   1. 先说出你在照片里看到了哪 3-5 件能指名道姓的具体东西（物件、动作、光线落点）；
   2. 每个涂鸦元素都必须呼应其中某一件——画在它旁边、指向它、描出它的形状、或表现它正在发生的动作。**说不出呼应哪一件的元素就不要画**；
   3. 文字若有，必须写出关于这张照片的一件具体事（地点、事件、日期、动作、心里话），不能只写物件名，也不能是换任何照片都能用的通用短语；
   4. 风格和配色贴合照片本身的气质与现场光（暖调照片用暖色，冷调用冷色，不要冷暖混用）；
   5. 不要遮挡面部五官。
   另外用文字告诉我：你看到的那 3-5 件具体东西是什么，以及每个元素分别呼应哪一件。"
```

生成约 1 分钟。产出 `doodled.png`。**它只是设计参考**——后续所有笔迹都由引擎重画。

让它同时回答"每个元素呼应哪一件"有两个用处：一是逼它先看照片再画（而不是套模板），二是它的回答直接就是你第 3 步复刻时的对照清单。**若它答不出呼应关系、只能给出"可爱装饰"这类泛泛描述，说明这张稿子没价值，弃用、改走自行设计。**
